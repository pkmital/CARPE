...
void ofxGui::draw()
{
	if(mIsActive)
	{
		...
		for(int i = 0; i < mObjects.size(); i++)
		{
			if( (ofxGuiObject*)mObjects.at(i)->active)
			{
				tmpObj = (ofxGuiObject*)mObjects.at(i);
				tmpObj->draw();
			}			
...
ofxGuiPanel* ofxGui::addPanel(int id, string name, int x, int y, int border, int spacing, bool alignToRight)
{
	ofxGuiPanel* newPanel = new ofxGuiPanel();
	newPanel->init(id, name, x, y, border, spacing, alignToRight);
