...
void ofxGuiButton::init(int id, string name, int x, int y, int width, int height, bool value, int display, bool alignToRight)
{
...
void ofxGuiButton::draw()
{
	glPushMatrix();
		...
		//if(alignRight)
		//	glTranslatef(mObjX-mGlobals->mHeadFont.stringWidth(mParamName), mObjY, 0.0f);
		//else	
		...
		if(mParamName != "")
		{
			if(alignRight)
			{
				drawParamString(-mGlobals->mParamFont.stringWidth(mParamName)-mCtrWidth-mCtrX-2-mCtrWidth, mGlobals->mButtonYText, mParamName, false);
			}
			else
			{
		...
		if(alignRight)
			ofRect(-mCtrWidth-mCtrX-mCtrWidth, mCtrY, mCtrWidth, mCtrHeight);
		else
...
{
	mMouseIsDown = isPointInsideMe(mouseToLocal(x, y), alignRight);
