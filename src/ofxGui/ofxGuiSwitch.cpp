...
void ofxGuiSwitch::init(int id, string name, int x, int y, int width, int height, int min, int max, int value, const string* paramStrings, bool alignToRight)
{
...
void ofxGuiSwitch::draw()
{
	...
	if(alignRight)
		glTranslatef(-mObjWidth, mObjY, 0.0f);
	else
	...
	if(mParamName != "")
	{
		...
		{
			string s = ofToString(mValue);
			drawParamString(0.0, 0.0, mParamName + ": " + s, false);
		}
	}
