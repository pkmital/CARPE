...
void ofxGuiPanel::init(int id, string name, int x, int y, int border, int spacing, bool alignToRight)
{
...
void ofxGuiPanel::draw()
{
	glPushMatrix();
		...
		if(mParamName != "")
		{
			if(alignRight)
			{
				drawHeadString(mObjWidth-mGlobals->mHeadFont.stringWidth(mParamName)-mBorder,
					mBorder, mParamName, false);
			}
			else
			{
		...
		if(alignRight)
			glTranslatef(mObjWidth-mBorder, 0.0f, 0.0f);
		for(int i = 0; i < mObjects.size(); i++)
		{
			tmpObj = (ofxGuiObject*)mObjects.at(i);
...
ofxGuiObject* ofxGuiPanel::addButton(int id, string name, int width, int height, bool value, int display, bool alignToRight)
{
	int offset = (mObjects.size() == 0 && mParamName == "") ? 0 : mSpacing;

	ofxGuiButton* button = new ofxGuiButton();
	button->init(id, name, mBorder, mObjHeight - mBorder + offset, width, height, value, display, alignToRight);
