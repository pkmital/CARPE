...
string ofxGuiObject::floatToString(float value, int display)
{
	...
	switch(display)
	{
		case kofxGui_Display_Int:
			stringValue = ofToString((int)value);
			break;
...
bool ofxGuiObject::isPointInsideMe(int x, int y, bool alignRight) 
{	
	if(alignRight)
	{
		int cx = -mCtrWidth-mCtrX-mCtrWidth;
		return (x >= cx && x <= cx+mCtrWidth && y >= mCtrY && y <= mCtrBottom);
	}
	else
...
{	
	return isPointInsideMe(p, false);
}
bool ofxGuiObject::isPointInsideMe(ofxPoint2f p, bool alignRight) 
{	
	if(alignRight)
	{
		int cx = mObjWidth-mCtrX;
		return (p.x >= cx-mCtrX && p.x <= cx+mObjWidth && p.y >= mCtrY && p.y <= mCtrBottom);
	}
	else
